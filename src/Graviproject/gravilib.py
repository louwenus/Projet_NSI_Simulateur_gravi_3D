...
G=6.674_30*10**-11
umPerKg=int(udPerMeter**3*G) #computing Kg weight based on udPerMeter and gravitation constant (c/lightspedd)³/G
...
class PyBaseDimension(cppgravilib.CyBaseDimension):
    ...
    def gerer_colision(self) -> None:
        ...
        for sphere, sphere2 in self.collisions():
            ...
            if sphere.durete==10**9:
                self.absorption(sphere, sphere2)
            elif sphere2.durete==10**9:
                self.absorption(sphere2, sphere)
            elif (sphere.get_rayon() > sphere2.get_rayon() * 3) or (sphere2.get_rayon() > sphere.get_rayon() * 3):
